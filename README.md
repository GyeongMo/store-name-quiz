...
> `initialism-quiz`(애니메이션 초성퀴즈)의 시스템 구조를 복제해 도메인만 향동 가게 이름(상호명)으로 바꾼 프로젝트. 아래 문서에 남아 있는 "애니메이션" 예시는 향동 가게 이름 예시로 치환해 읽으면 된다.
...
| 사용자 커스텀 퀴즈 · 설정 · 사운드 · 최근 결과 | `localStorage` 키 `store-name-quiz-v1` |
| Electron 사용자 데이터 | `%APPDATA%\상호명 초성 퀴즈\Local Storage\leveldb\` (Windows) |
| 게임 결과 수동 저장 | 사용자 선택 경로 (기본: `Documents/상호명초성퀴즈/`) |
...
이 소프트웨어는 **교육 목적으로 개발되어 [MIT License](LICENSE)로 공개된 오픈소스 소프트웨어**입니다. 누구나 자유롭게 사용·수정·재배포할 수 있습니다.

다만 **소스 코드의 라이선스와 별개로**, 게임 콘텐츠에 사용된 향동 가게 이름(상호명)·명대사 등은 각 권리자의 저작권·상표권 대상일 수 있습니다. 외부 배포·상업적 이용 시에는 해당 콘텐츠의 권리 정책을 별도로 확인하세요.

```
MIT License — Copyright (c) 2026 GyeongMo
```
